...
class OpenAIService
{
    ...
    public function transcribe(UploadedFile $audioFile): string
    {
        if (empty($this->apiKey)) {
            Log::info('OpenAI API key not set — using mock transcription');
    ...
    {
        if (empty($this->apiKey)) {
            Log::info('OpenAI API key not set — skipping GPT extraction');
            return [
                'customer_name' => null,
                'action_taken' => null,
                'amount' => null,
        ...
        try {
            ...
            Log::warning('GPT extraction failed, relying on heuristics', ['error' => $e->getMessage()]);
            return [
                'customer_name' => null,
                'action_taken' => null,
                'amount' => null,
